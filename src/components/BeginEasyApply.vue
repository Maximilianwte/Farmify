<template>
    <div id="beginEasyApply"
        class="flex-col text-2xl absolute mt-24 px-12 bg-bg_primary border-2 border-main_primary rounded-sm w-1/2"
        style="z-index: 200">
        <h5 class="mt-12 text-3xl">Begin Easy Apply</h5>
        ...
        <p class="text-justify text-lg mt-2">To start using Easy Apply, one of our most awesome functions, we need some
            information to personalize the
            messages.</p>
        <div id="input" class="text-xl flex-col">
            <li>
                <ul class="mt-8 flex items-center">
                    <p class="w-48 text-right mr-4">Since when are you in Australia?</p>
                    <input type="text" placeholder="Date" v-model="input.inAustraliaSince"
                        class="rounded-sm w-90 border-2 border-main_secondary text-bg_primary px-2 py-2">
                </ul>
                <ul class="mt-6 flex items-center">
                    <p class="w-48 text-right mr-4">What is your favorite memory that you have from here?</p>
                    <input type="text" placeholder="Memory" v-model="input.memory"
                        class="rounded-sm w-90 border-2 border-main_secondary text-bg_primary px-2 py-2">
                </ul>
                <ul class="mt-6 flex items-center">
                    <p class="w-48 text-right mr-4">Where did you arrive in Australia?</p>
                    <input type="text" placeholder="Place" v-model="input.arrivedAt"
                        class="rounded-sm w-90 border-2 border-main_secondary text-bg_primary px-2 py-2">
                </ul>
            </li>
            <button
                class="button mb-8 mt-10 bg-main_focus hover:bg-main_focus_active text-main_primary py-2 px-8 lg:px-8 rounded mx-2 cursor-pointer">Accept</button>
        </div>
    </div>
</template>
<script>
