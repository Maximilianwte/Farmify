<template>
    <div class="list-item relative py-2 text-xl bg-bg_primary text-main_primary rounded-sm w-5/6 lg:w-128">
        ...
        <h2 class="px-4 py-1 text-bg_secondary">{{item.adress}}</h2>
        ...
        <div class="below w-full flex items-center justify-end mt-12 py-1 relative cursor-pointer">
            <div v-if="item.phone != undefined" id="call" title="Phone" alt="Phone" class="cursor-pointer px-1">
                <div @click="handleButtonClick('phone', item)" @mouseleave="handleActiveButton(0)" class="button">
            ...
            <div v-if="item.website != undefined" id="website" title="Website" alt="Website"
                ...
                <div @click="handleButtonClick('website', item)" @mouseleave="handleActiveButton(0)" class="button">
            ...
            <div v-if="item.email != undefined" id="mail" title="Email" alt="Email"
                class="cursor-pointer relative px-1">
                <div @click="handleButtonClick('mail', item)" @mouseleave="handleActiveButton(0)" class="button">
            ...
            <div @click="getEasyApply(item.id)" @mouseleave="handleActiveButton(0)" class="relative"
            ...
            <div class="saveFarm ml-4 mt-3 absolute bottom-0 left-0 md:mb-2">
                <svg @click="handleSavedFarm()" @mouseleave="handleActiveButton(0)" alt="Save Farm" title="Save Farm"
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getDistance() {
                var distance = farm_functions.calculateDistance(store.state.geoCode, this.item.location);
                return distance
            },
            handleButtonClick(id, item) {
                switch (id) {
                    case "phone": {
                        this.auth ? window.open("tel:+" + item.phone, "_self") : '';
                        this.handleActiveButton(1);
                        break;
                    }
                    case "website": {
                        this.auth ? window.open('https://' + item.website) : '';
                        this.handleActiveButton(2);
                        break;
                    }
                    case "mail": {
                        this.auth ? window.open("mailto:" + item.mail, "_self") : '';
