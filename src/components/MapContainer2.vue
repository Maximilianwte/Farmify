<template>
  <div>
    <vl-map :load-tiles-while-animating="false" :load-tiles-while-interacting="true" :logo="false"
      data-projection="EPSG:4326" class="h-full">
      <vl-view :zoom.sync="zoom" :min-zoom="4.6" :center.sync="center" :rotation.sync="rotation"></vl-view>
      ...
      <vl-overlay v-for="item in markerData" :key="item.id" :id="item.id" :position="item.location" :offset="handleOffset(item.id)">
        ...
        <template class="relative" v-if="activeMarker == item.id">
          <div @click="setActiveMarker(item)" class="close cursor-pointer text-xl w-8 h-8 ml-90 flex-col bg-main_focus rounded-full">x</div>
          <div style="width: 340px; height: 225px; z-index: 20"
    ...
    methods: {
      ...
      setActiveGroup(item) {
        ...
        if (this.activeGroup != item.id) {
          ...
          //this.center = [item.location[0], item.location[1] + (90 / this.zoom ** 3)];
        } else {
          this.activeGroup = 0;
        }
