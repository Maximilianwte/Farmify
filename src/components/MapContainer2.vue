<template>
  <div>
    <vl-map
      @click="activeMarkerValue == 0 ? setActiveGroup(0) : null; activeMarkerValue = 0;"
      :load-tiles-while-animating="false"
      :load-tiles-while-interacting="true"
      data-projection="EPSG:4326"
      class="h-full bg-main_primary"
    >
      <vl-view
        :zoom.sync="zoom"
        :min-zoom="3.5"
      ...
      >
        <div id="header" class="mt-1 ml-2">
          <router-link class="mr-4 flex items-center" to="/">
            <svg class="svg-lg svg-dark" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
              <path
      ...
      <div
        class="buttons absolute bottom-0 left-0 mb-12 ml-4 flex text-xl cursor-pointer"
        style="z-index: 300"
      >
        <div
          @click="handleZoom('in')"
          id="zoomIn"
          class="px-4 py-2 bg-bg_primary hover:bg-bg_secondary text-main_primary rounded-l-sm"
        >+</div>
        <div
          @click="handleZoom('out')"
          id="zoomIn"
          class="px-4 py-2 bg-bg_primary hover:bg-bg_secondary text-main_primary rounded-r-sm"
        >-</div>
      </div>
      ...
      >
        <button
          ...
          class="group px-5 cursor-pointer rounded-full border-2 border-bg_primary text-xl text-bg_primary bg-main_primary hover:bg-blue_active"
        >{{getActiveNumberInGroup(item)}}</button>
      </vl-overlay>
            ...
            >
              ...
              <div class="textBox absolute w-5/6 h-full">
                ...
                <div class="saveFarm ml-2 mt-2">
                  <svg
                    alt="Save Farm"
                    title="Save Farm"
                    :style="getFill(item.id)"
                    @click="handleSavedFarm(item.id)"
                    class="svg-mds cursor-pointer"
                    viewBox="0 -10 512 511"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
              ...
              >
                ...
                >
                  <a :href="'mailto:' + item.email">
                    <svg viewBox="0 0 448 448" xmlns="http://www.w3.org/2000/svg">
                      ...
                      <path
                        d="M224 240a8 8 0 01-5.2-2L120 153.5V304h208V153.4L229.2 238a8 8 0 01-5.2 1.9zm0 0"
                      />
                      <path
                ...
                <div v-if="item.email != undefined" id="easyContact">
                  <button
                    class="bg-main_primary cursor-pointer hover:bg-main_secondary text-bg_primary rounded-sm text-lg px-4 py-2 ml-2 mr-2"
                  >
                    Easy
                    Apply
                  </button>
          ...
          <template v-else>
            ...
            >
              ...
              <div class="textBox absolute w-5/6 h-full">
                ...
                <div class="saveFarm ml-2 mt-2">
                  <svg
                    ...
                    :style="getFill(item.id)"
                    @click="handleSavedFarm(item.id)"
                    @mouseleave="handleActiveButton(0)"
                    class="svg-mds cursor-pointer"
                    viewBox="0 -10 512 511"
                    xmlns="http://www.w3.org/2000/svg"
                  >
              ...
              >
                ...
                <div v-if="item.email != undefined" id="easyContact">
                  <button
                    @click="handleActiveButton(4)"
                    @mouseleave="handleActiveButton(0)"
                    class="bg-bg_secondary cursor-pointer hover:bg-main_secondary text-bg_primary rounded-sm text-lg px-4 py-2 ml-2 mr-2"
                  >
                    Easy
                    Apply
                  </button>
...
export default {
  ...
  computed: {
    ...
    fetchFarmIDsActive() {
      return store.state.farms.activeFarmIDs;
    }
  },
  mounted() {
  ...
  watch: {
    ...
    /*     center: function() {
      ...
      if (
        ...
        Math.abs(this.center[1] - this.last_center[1]) > step
      ) {
        this.calculateBoundaries();
        this.calculateGroups();
  ...
  methods: {
    ...
    getActiveMarker(item) {
      if (this.farms_in_groups.includes(item.id)) {
        ...
        if (
          this.fetchFarmIDsActive != null &&
          !this.fetchFarmIDsActive.includes(item.id)
        ) {
          return false;
        } else {
          if (this.activeMarkerValue == item.id) {
    ...
    calculateGroups() {
      this.markerData = store.state.farms.active;
      this.groupData = farm_functions.groupFarms(
        this.markerData,
        this.boundaries,
        this.zoom
